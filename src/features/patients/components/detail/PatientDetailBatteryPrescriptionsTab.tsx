...
function toDeliveryRowView(r: BatteryPrescriptionDeliveryRow): DeliveryRowView {
  ...
  const qtyBoxes = (anyRow.qty_boxes ?? anyRow.qtyBoxes ?? null) as
    | number
    | null;
  const qtyPacks = (anyRow.qty_packs ?? anyRow.qtyPacks ?? null) as
    | number
    | null;
  const qtyUnits = (anyRow.qty_units ?? anyRow.qtyUnits ?? null) as
    | number
    | null;
  const prescriptionNo = (anyRow.prescription_no ?? anyRow.prescriptionNo ??
    null) as string | null;
  const sgkExpectedAmount = (anyRow.sgk_expected_amount ??
    anyRow.sgkExpectedAmount ??
    null) as number | null;
...
export function PatientDetailBatteryPrescriptionsTab({
  ...
  const deliveriesQuery = useQuery({
    queryKey: open
      ? DELIVERIES_QUERY_KEY(patientId)
      : ['battery-prescription-deliveries', 'closed'],
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const orgId = readOrgId(profile);
      if (!orgId) {
        throw new Error(
          'ORG_ID_MISSING: Profil org_id bulunamadı. Lütfen tekrar giriş yapın.',
        );
      }
      ...
      const batteryType = form.batteryType;
      if (!batteryType) {
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: DELIVERIES_QUERY_KEY(patientId),
      });
    },
  });
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <label className="space-y-1 sm:col-span-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, sgkExpectedAmount: e.target.value }))
                }
