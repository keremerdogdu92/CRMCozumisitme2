...
//
// Patch v2.1:
// - FIX: Build error TS2353 ("'file' does not exist in type 'File'").
//   The mutation expects a File, but an object literal like { file } was being passed.
//   Now we call mutateAsync(file) directly.
// - Polish: disable submit when file is missing; avoid stale summary on new selection.
...
export function InventoryImportCard({ open, onToggle }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // FIX: mutationFn expects File, not { file: File }
