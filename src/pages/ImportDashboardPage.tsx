// src/pages/ImportDashboardPage.tsx
// Summary: Central "Import Fix Center" dashboard for patients and legacy device imports.
// Shows import_jobs by target_entity, job summaries, and error rows with per-row fix modals.
...
} from '../features/patients/import/types';
import {
  fetchPatientsImportErrorRows,
  fetchLegacyDevicesImportErrorRows,
  getPatientsImportJobSummary,
  getLegacyDevicesImportJobSummary,
} from '../features/patients/import/api.jobs';
import { LegacyDeviceRowFixModal } from '../features/patients/import/LegacyDeviceRowFixModal';
import { PatientRowFixModal } from '../features/patients/import/PatientRowFixModal';
...
export function ImportDashboardPage() {
  ...
  async function refreshSelectedJob(job: ImportJobRow | null) {
    ...
    try {
      ...
      setSummaryError(
        (err as Error)?.message || 'Job özeti alınırken hata oluştu.',
      );
      setRowsError(
        (err as Error)?.message || 'Hata satırları alınırken hata oluştu.',
      );
    } finally {
  ...
  return (
    <PageLayout>
      <PageHeader
        title="Import Fix Center"
        subtitle="Hasta ve legacy cihaz CSV import job'larını, hata satırlarını ve basit düzeltmeleri tek ekrandan yönet."
      />
              ...
              <>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="rounded-md border border-slate-300 px-2 py-1 text-[10px] font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                ...
                {jobSummary && (
                  <div className="mb-3 rounded-md bg-slate-50 p-2 text-[10px] text-slate-800">
                ...
                {!rowsLoading && showPatientErrors && patientErrorRows.length > 0 && (
                  <div className="mt-1 max-h-80 overflow-y-auto rounded-md border border-slate-100">
                ...
                {!rowsLoading && showLegacyErrors && legacyErrorRows.length > 0 && (
                  <div className="mt-1 max-h-80 overflow-y-auto rounded-md border border-slate-100">
                    <table className="min-w-full border-collapse text-[11px]">
                      <thead className="bg-slate-50">
                        <tr>
