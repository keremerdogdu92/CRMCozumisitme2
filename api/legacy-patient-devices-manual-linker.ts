...
// It links the row to a specific patient_id and creates devices + patient_devices records.
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const stagingRowId =
      (req.body && typeof req.body.staging_row_id === 'string' && req.body.staging_row_id) ||
      (typeof req.query.staging_row_id === 'string'
        ? req.query.staging_row_id
        : null);

    const patientId =
      (req.body && typeof req.body.patient_id === 'string' && req.body.patient_id) ||
    ...
    const soldAtIso = parseDateLike(rr.sold_at);
    ...
    // 3) Ensure patient belongs to the same org (safety)
    ...
    if (patientError) {
      res.status(500).json({
        error: 'Failed to load patient for manual link: ' + patientError.message,
      });
      return;
    }
    ...
    // 4) Insert devices + patient_devices
    ...
    const assignSideAndInsert = async (side: 'left' | 'right') => {
      ...
      const { data: devData, error: devError } = await supabase
        ...
        .insert({
          ...
          serial: serial ?? null,
          status: 'sold',
          hold_patient_id: patientId,
        })
        .select('id')
        .single();

      if (devError) {
        throw new Error('Failed to insert device: ' + devError.message);
      }

      const deviceId = (devData as any).id as string;
    ...
    try {
      ...
      res.status(500).json({
        error:
          'Failed to create device / patient_devices records: ' +
    ...
    if (updateError) {
      res.status(500).json({
        error:
          'Devices created, but failed to update staging row: ' +
          updateError.message,
      });
      return;
    }
